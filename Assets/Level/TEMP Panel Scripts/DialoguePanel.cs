...
public class DialoguePanel : MonoBehaviour, IPointerClickHandler
{
    ...
    public void HidePanel()
    {
        ...
        activeBox.transform.DOScale(Vector3.zero, CHARACTER_ANIMATION_TIME).SetEase(Ease.OutQuart);
        _characterNameBox.transform.DOScale(Vector3.zero, CHARACTER_ANIMATION_TIME).SetEase(Ease.OutQuart);
        _characterImage.transform.DOMoveY(_characterOffscreenPositionY, CHARACTER_ANIMATION_TIME).SetEase(Ease.OutQuart);
    }
    ...
    private void SwapCharacters(Sprite characterSprite)
    {
        ...
        _characterImage.transform.DOMoveY(_characterOffscreenPositionY, CHARACTER_ANIMATION_TIME).SetEase(Ease.OutQuart).OnComplete(() =>
        {
            _characterImage.sprite = characterSprite;
            _characterImage.transform.DOMoveY(_characterDefaultPositionY, CHARACTER_ANIMATION_TIME)
            .SetEase(Ease.OutQuart).OnComplete(() => _isAnimationFinished = true);
        });
    }
